...
export default function Page() {
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (resp && resp.status) {
        router.push("/login");
        toast('Please login using the credentials')
      } else {
        toast(resp.message);
      }
    } catch (error) {
  ...
  return (
    ...
    <div>
      <div className="p-6 flex items-center justify-between border">
        <Link href="/">
          <AudioLines height={30} width={30} className="text-rose-700"/>
        </Link>
      </div>
      <div className="w-full lg:grid lg:min-h-screen lg:grid-cols-2 xl:min-h-screen">
        ...
        <div className="hidden bg-muted lg:block">
          <h1 className="flex h-full items-center justify-center text-[50px] font-bold">
            Music is the universal <br /> Language of mankind
          </h1>
        </div>
      </div>
    </div>
